...
INPUT_DATASET = "data/preprocessed/rep_dataset_bicep_curls_v1.csv"
OUTPUT_DATASET = "rep_dataset_bicep_curls_features.csv"

RAW_SET_FOLDER = "data/bicep_curl"
...
#I need to clean this code up, some no longer useful funcs
...
def jerk_features(x, fs, prefix):
    """
    Jerk = change in signal over time.
    This can capture shakiness/instability.
    """
    x = np.asarray(x, dtype=float)
    x = x[~np.isnan(x)]
...
def frequency_domain_features(x, fs, prefix):
    ...
    #useful movement/fatigue bands for slow reps
...
def extract_features_from_rep_segment(rep):
    ...
    #not generating ay_mean, ay_std, ay_min, ay_max, or ay_range since that is already in the dataset
    signals = {
        "ax": ax,
        "az": az,
        "gx": gx,
    ...
    #metadata about segment quality
...
def add_baseline_features(df, baseline_reps=3):
    ...
    #features that are especially meaningful relative to early set baseline
