...
Requires real_time_predictor_bicep_curl.py and the bicep model bundle
...
Examples:
  ...
  python plot_imu_sets.py --port /dev/cu.usbserial-210
  python plot_imu_sets.py --csv data/bicep_curl/set_01.csv
  python plot_imu_sets.py --port /dev/cu.usbserial-210 --threshold 0.30
"""
...
from real_time_predictors.real_time_predictor_bicep_curl import (
    parse_serial_line,
...
def plot_final_set(buffer, detected_reps, output_path="final_bicep_set_segmentation.png"):
    """
    ...
    if "time_s" not in df.columns or "ay_g" not in df.columns:
        print("Missing time_s or ay_g, cannot plot final set.")
        return

    time_s = df["time_s"].values
    ay = df["ay_g"].values
    smooth = smooth_signal(ay, SMOOTH_WINDOW)
    ...
    plt.title("Final Bicep Curl Set Segmentation")
    plt.xlabel("Time (s)")
    plt.ylabel("ay_g")
    plt.legend()
    plt.grid(True)
        ...
        for rep_info in completed:
            ...
            if skip:
                with _lock:
                    ...
                    # keep a small margin before rep start in the rolling classifier buffer
                    KEEP_BEFORE_SEC = 0.5
...
class GUI:
    def __init__(self, exercise: str, window: float, threshold: float):
        ...
        self.ax_prob.set_title(
            f"Near-Failure Probability  (threshold = {self.threshold:.2f})",
            fontsize=8,
            color=C["text"],
            pad=4,
        )
        ...
        self._hdr = self.fig.text(
            0.5,
            0.965,
            f"BICEP CURL MONITOR  ·  {exercise.upper()}  ·  threshold={self.threshold:.2f}  ·  press R to start recording",
            ha="center",
            va="top",
            fontsize=9,
            color=C["sub"],
        ...
        if k == "r":
            ...
            if not was:
                ...
                plot_final_set(
                    final_buffer,
                    final_detected_reps,
                    output_path="final_bicep_set_segmentation.png",
                )
    ...
    def update(self, _frame):
        ...
        if not rec:
            self._set_hdr(
                f"BICEP CURL MONITOR  ·  {self.exercise.upper()}  ·  threshold={self.threshold:.2f}  ·  {status}  ·  press R to record",
                C["sub"],
            )
        ...
        if last_pb is not None:
            col = C["red"] if last_pb >= self.threshold else C["green"]
        ...
        if last_pb is not None:
            ...
            self._t_prob.set_color(C["red"] if last_pb >= self.threshold else C["green"])
        else:
        ...
        if rec:
            if base_n < BASELINE_REPS:
                self._set_hdr(
                    f"● RECORDING  ·  threshold={self.threshold:.2f}  ·  building baseline  {base_n}/{BASELINE_REPS} reps",
                    C["orange"],
                )
            else:
                self._set_hdr(
                    f"● RECORDING  ·  threshold={self.threshold:.2f}  ·  {counted} reps  ·  press R to stop",
                    C["rec"],
                )
...
def main():
    ...
    print(f"Using near-failure threshold: {threshold:.2f}")
